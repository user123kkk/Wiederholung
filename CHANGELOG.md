## 3.9.7 – 23. September 2026

**Beobachtung 21, zweite Hälfte: eigenes Grammatik-Feld für Karten.** Bisher trug das Feld „Beispielsatz, Bild-Link oder Notiz" (`extra`) auch Grammatikangaben, nur per Label-Wort kenntlich gemacht (v3.8.5). Jetzt ein eigenes, kurzes Feld (`grammatik`, max. 200 Zeichen – ein Schlagwort wie „Perfekt, 3. Pers. Sg. m.", kein Satz): eigenes Eingabefeld im Karten-Formular zwischen Übersetzung und Notiz, sichtbar in der Verwalten-Liste (kompakte Vorschauzeile wie bei der Notiz), in der Detailansicht, im Lernen-Tab-Raster und im Übungsmodus (dort erst nach dem Aufdecken der Antwort, neben der Übersetzung). Durchsuchbar wie Wort/Übersetzung/Notiz, mitgeführt in Weitergabe-Dateien und beim Zusammenführen aktualisierter Kartensätze. `firestore.rules` entsprechend erweitert (`kartenFelder()`, `kartenWerte()`), dieselbe 200-Zeichen-Grenze wie in `app.js`. Mit Playwright end-to-end geprüft: Anlegen, Anzeige an allen vier Stellen, Suche, erneutes Bearbeiten – alles bestätigt gegen eine Firebase-Attrappe mit echtem Schreiben/Lesen. Die Firestore-Regeln selbst liefen mangels Emulator in dieser Umgebung nur durch sorgfältigen Codevergleich mit der bereits geprüften `extra`-Regel, nicht gegen den echten Emulator.
