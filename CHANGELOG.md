...
Der Umfang ist nicht geraten, sondern gefiltert. Grundlage sind zwei vom Betreiber gelieferte Videos, beide zweimal ausgewertet (Untertitel und Einzelbilder) und in `plan/onboarding/VIDEO-BEFUND.md` und `VIDEO-BEFUND-2.md` getrennt nach Auszählung, Einzelfall und Muster abgelegt. Aus 139 aufgeschriebenen Fragenkandidaten sind **drei** Einstellfragen übrig geblieben — mehr kann die App heute schlicht nicht einstellen (`normSettings`: `arabGroesse`, `thema`, `sitzungsLimit`); 101 Kandidaten sind mit Begründung abgelehnt, nicht weggelassen. Die Ablehnungsgründe stehen als sieben Prüfungen in `plan/onboarding/FRAGENKATALOG.md`.

**Was der Einstieg bewusst nicht tut**, weil beide Videos es empfehlen und beide Male abgelehnt wurde: keine Frist-Zusage („in 30 Tagen kannst du …"), keine absichtliche Länge, damit am Ende der versunkene Aufwand wirkt, keine Frage, deren einziger Zweck das Überzeugen ist, kein vorgefüllter Fortschrittsbalken, kein Zähler, kein Ausrufezeichen. Begründung je Punkt in `plan/onboarding/PSYCHOLOGIE.md`. „Überspringen" steht auf jedem Bildschirm gleichrangig neben dem Hauptknopf, nicht kleiner und nicht grau.

**Technisch:**

- Die Antworten liegen bis zur Registrierung im `localStorage` (`adrabic-einstieg-antworten`), nicht in `settings`. Grund: `onAuthStateChanged` setzt `settings` bei jedem Anmeldewechsel unbedingt zurück — was nur im Arbeitsspeicher stünde, wäre beim Anmelden weg, also genau dann, wenn es gebraucht wird.
- Angewendet wird **nur bei einem frischen Konto**: im Zweig, in dem Firestore kein Nutzerdokument liefert (`cloudDocExists === false`). Wer sich auf einem neuen Gerät in ein **bestehendes** Konto einloggt, bekommt keine Einstellung überschrieben. Danach wird der Zwischenspeicher gelöscht.
- Das Thema ist die Ausnahme und braucht keinen neuen Speicher: `setThema()` schreibt `adrabic-thema` schon bisher ohne Konto, und das Kopfskript in `index.html` liest es vor der ersten Zeichnung.
- Der Merker `adrabic-einstieg` bleibt nach einer Abmeldung stehen — wer sich abmeldet, ist kein Neuling. Fällt `localStorage` aus (privates Fenster, gesperrte Website-Daten), gilt der Einstieg als gesehen: lieber gar keiner als einer, der bei jedem Öffnen wiederkommt und nichts behalten kann.
- **Die Lernlogik ist nicht angefasst.** Der Probelauf ist eine Anzeige: die Bewertung wird nicht gespeichert, es entsteht keine Karte, keine Stufe, kein Verlaufseintrag. `sitzungsLimit` ist die Rundengröße, keine Änderung daran, was gelernt oder wie bewertet wird.
- Gestalt aus dem Bestand: `.solo` wie Anmelden und Einrichtung, `.seg` wie die Wahl-Blätter in den Einstellungen, ein delegierter Klick-Listener über `data-action`, **eine** Eintrittsbewegung pro Bildschirm als `@keyframes` (`.anim-rise`, 200 ms), `prefers-reduced-motion` greift über die bestehende Regel. Neu in `styles.css` ist nur, was es sonst nirgends gibt: die Probefläche und die Ankerliste (Abschnitt 16b).

**Geprüft** im Browser über alle sieben Bildschirme, hell und dunkel, 1024 px und 375 px: keine Konsolenfehler, kein waagerechtes Scrollen, Schriftgröße und Thema wirken sofort sichtbar, der Wenn-dann-Satz zieht die Auswahl nach, nach „Konto anlegen" stehen Merker und Antworten korrekt im `localStorage`. **Nicht geprüft** und deshalb offen: der Durchlauf mit echter Firebase-Anmeldung (braucht ein echtes Konto) und der Test am Gerät des Betreibers.

**Offen, vom Betreiber zu erledigen:** die Rechtsprüfung der Zwischenspeicherung vor der Registrierung (`datenschutzerklaerung.html` Punkte 6 und 9 nennen `localStorage` heute „rein technisch notwendig" für Thema und Einstellungen; die beiden neuen Schlüssel verlängern das) und das Wort auf der Beispielkarte — im Moment steht dort كِتَابٌ (Buch) als Platzhalter.

## 3.9.8 – 23. September 2026

**Eigenes Grammatik-Feld (v3.9.7) wieder entfernt.** Betreiber-Entscheidung, wenige Stunden nach dem Bau: das Feld war 0 von 136 realen Karten genutzt, stand aber als eigenes, immer sichtbares Formularfeld zwischen Übersetzung und Notiz (Grammatik-Regel als Faustregel bemüht: mehr sichtbare Eingabeoptionen kosten Aufmerksamkeit, auch wenn optional). Zurück auf den Stand von v3.8.5: Grammatik-Hinweise stehen wieder im „Notiz"-Feld (`extra`), kenntlich gemacht über das Label „Beispielsatz, Grammatik, Bild-Link oder Notiz". Kein eigenes `grammatik`-Feld mehr in `normCard`, Cloud-Schreiben/-Lesen, Formular, Weitergabe-Format, Abgleich/Zusammenführen, Suche oder `firestore.rules`. Bestehende Karten, deren `grammatik`-Feld in der Cloud schon befüllt wurde (laut geprüftem Export keine), würden beim nächsten Speichern stillschweigend keinen Effekt mehr zeigen – bei 0 betroffenen Karten kein reales Risiko.

## 3.9.6 – 23. September 2026

**Beobachtung 12 (kurzer leerer Bildschirm vor dem Lade-Indikator) behoben.** Der drehende Ladekreis (`.boot__orbit`, drei Ringe) wurde bisher erst von `render()` in `app.js` erzeugt – bis das Skript geladen und ausgeführt war, stand nur das stille App-Symbol da, ohne jede Bewegung. `index.html` zeigt jetzt von der ersten Zeichnung an **exakt dasselbe Markup**, das `render()` für denselben Ladezustand baut (dieselben drei Ringe, dasselbe Symbol `flower-isolated.png`, derselbe Text „Deine Karten werden geladen…") – rein aus HTML/CSS (`@keyframes bootSpin`/`bootSpinRev`, bereits vorhanden), ganz ohne JavaScript. Geprüft mit Playwright bei komplett blockiertem `app.js`: Ring, Symbol und Text stehen sofort da und drehen sich messbar (Transform ändert sich innerhalb von 400 ms). Übernimmt `render()` danach, ist die Animation schon in Bewegung – kein Sprung, kein Aufblitzen. Nebenbei eine kleine Abweichung behoben: Das bisherige statische Symbol war `desktop-icon.png`, `render()` zeigt `flower-isolated.png` – beim Übergang wäre also ohnehin ein sichtbarer Symbolwechsel gewesen.

## 3.9.5 – 23. September 2026

**Prozess-Lücke gefunden und geschlossen: `index.html`-Versions-Query bei den letzten beiden Veröffentlichungen (3.9.3, 3.9.4) übersehen.** `README.md` verlangt seit dem 18.09.2026-Fund (ein Syntaxfehler blieb wegen des HTTP-Caches bis zu eine Stunde live) drei Schritte, nicht nur zwei: `APP_VERSION`, `CACHE_NAME` **und** den `?v=…`-Query am `<script src="./app.js?v=…">` in `index.html`. `CLAUDE.md`s eigene Kopie dieser Liste hatte den dritten Schritt nie übernommen – dadurch blieb `index.html` bei 3.9.2 stehen, während `app.js`/`sw.js` schon auf 3.9.4 liefen. Ohne Wirkung auf Nutzer:innen, die ohnehin `index.html` selbst offen hatten (die bekamen `app.js` per Cache-Busting trotzdem neu, siehe Kommentar in `index.html`) – aber ein neuer Besuch innerhalb der Stunde nach 3.9.3/3.9.4 hätte `app.js` bis zu eine Stunde lang aus dem normalen HTTP-Cache des Browsers bedient (`Cache-Control: max-age=3600`), nicht aus dem Service-Worker-Cache. Jetzt nachgezogen, `CLAUDE.md` um den fehlenden Schritt ergänzt, damit das keiner künftigen Session mehr passiert.

## 3.9.4 – 23. September 2026

**Beobachtung 16 (Firebase-Fehler nach Browser-Zurück): echte Ursache für den nutzlosen Wiederholungsversuch gefunden und behoben.** Seit v3.0.41–43/51 gab es mehrere Verdachtsfixe ohne bestätigte Wirkung – dieser hier ist erstmals ein **nachgewiesener** Fehler, kein Verdacht: `importMitVersuch()` (`app.js`) sollte einen fehlgeschlagenen `import()` eines Firebase-Bausteins bis zu dreimal mit derselben URL wiederholen. Mit Playwright nachgestellt (ein lokaler Server, der die erste Anfrage kappt und danach normal antwortet): Chromium cacht eine fehlgeschlagene Modul-Auflösung für eine exakte URL – jeder weitere `import()` derselben Zeichenkette lehnt sofort ab, **ohne überhaupt eine neue Netzwerkanfrage zu stellen**. Die „drei Versuche" waren dadurch seit ihrer Einführung faktisch nie mehr als einer; ein kurzer, an sich erholbarer Netzwerk-Aussetzer (genau das Szenario aus Beobachtung 16) hatte dadurch nie eine echte zweite Chance. Fix: ab dem zweiten Versuch bekommt die URL einen zählenden Anhang (`?wiederholung=2`, `?wiederholung=3`) – für den Modul-Cache eine neue, unbekannte Adresse, für den Server (gstatic.com, per CSP nur nach Ursprung erlaubt) dieselbe Datei. Mit demselben Testaufbau bestätigt: vorher 1 echter Netzwerkversuch trotz dreifacher Schleife, danach 2 (der zweite gelingt). Zusätzlich end-to-end gegen die echte `app.js`/`initFirebase()` geprüft: ein einmalig gekappter Ladeversuch für einen Firebase-Baustein führt jetzt zum normalen Anmeldebildschirm statt zum Fehlerbildschirm.

Bewusst ehrlich: Dieser Fund erklärt einen echten, jetzt behobenen Defekt in der Wiederholungslogik – ob er die **einzige** Ursache der ursprünglich gemeldeten Fehlermeldung war, lässt sich ohne echten Gerätetest (Browser-Zurück-Navigation auf einem echten Telefon) nicht abschließend sagen. Die bestehende Selbstheilung (kompletter Neuladen-Notnagel, max. 2 automatische Versuche) bleibt als weiteres Sicherheitsnetz unverändert bestehen.

**Beobachtung 13 (Über-Scrolling) noch einmal durchgesehen, kein weiterer Fund.** Der bestehende `dvh`→`svh`-Fix (v3.0.50) deckt den beschriebenen Mechanismus (wachsender Leerraum, wenn die Werkzeugleiste des mobilen Browsers während des Scrollens einklappt) bereits vollständig ab – keine verbliebene `100dvh`/`100vh`-Stelle gefunden, `--vv-gap` (Beobachtung 18) betrifft nachweislich nur die Positionierung der Navigationsleiste, nicht die Seitenhöhe. Kein Code geändert – ein weiterer Eingriff ohne echten Gerätetest mit ein-/ausklappender Werkzeugleiste (in dieser Umgebung nicht nachstellbar) wäre wieder nur eine unbelegte Vermutung.

## 3.9.3 – 23. September 2026
...
**Zweiter echter Fund im Feedback-Board, direkt im Anschluss an 3.9.1: „Lädt…" konnte trotz des Endlosschleifen-Fixes dauerhaft stehen bleiben.** Betreiber-Meldung mit neuem Screenshot: auf einem Konto blieb die Liste dauerhaft bei „Lädt…", obwohl auf einem anderen Konto kurz zuvor ein Vorschlag erfolgreich eingereicht wurde – also kein Regel-/Deploy-Fehler (der hätte sofort die Fehlermeldung aus 3.9.1 gezeigt), sondern ein echter Netz-Hänger: die Antwort auf die Abfrage kam nie an, weder als Erfolg noch als Fehler, und ohne Zeitlimit wartete die Seite ewig darauf. Fix, dasselbe Muster wie beim Start-Ladebildschirm seit 2.21.1 (`render()`, `ladeTimer`/`ladeLangsam`): Nach 9 Sekunden ohne Antwort erscheint „Das dauert länger als sonst" mit einem „Erneut versuchen"-Knopf, der einen wirklich neuen Versuch startet – ein spät doch noch eintreffendes Ergebnis des aufgegebenen Versuchs wird über eine laufende Nummer (`feedbackLadeToken`) erkannt und verworfen, statt einen inzwischen neueren Stand zu überschreiben.

Bei dieser Gelegenheit auf gleichartige Mängel im übrigen Werkzeug durchgesehen: Das Feedback-Board war die einzige Stelle mit einer unbegrenzt wartenden „Lädt…"-Anzeige ohne Zeitlimit – der Start-Ladebildschirm, alle Formulare (Anmelden/Registrieren/Passwort zurücksetzen/Vorschlag einreichen) und die Code-Einlöse-Wege für geteilte Lektionen fragen entweder über eine blockierende Abfrage (Fehler wird sofort gemeldet) oder haben bereits ein eigenes Zeitlimit. Kein weiterer Fund.
...
## 3.8.7 – 23. September 2026

**Schwebende Leisten (Navigation, AppBar, Modusleiste) sahen auf leeren Bildschirmen sichtbar dunkler aus als über Karten.** Betreiber-Screenshots (leerer Bereich in Verwalten vs. voller Fortschritt-Bildschirm) zeigten den Unterschied direkt nebeneinander. Ursache gefunden und nachgemessen: Die drei Leisten sind halbtransparent mit Weichzeichner (`backdrop-filter: blur`) – über Karten (heller) blendet der Weichzeichner das durch, über leerem Hintergrund (dunkler) eben das. Nachgemessen: `.nav` lag über Karten bei rgb(48,47,45), über leerer Fläche bei rgb(25,23,21) – fast doppelt so dunkel, derselbe Bauteil. `--surface`/`--bg`-Anteil in allen drei Leisten von 82–86 % auf 95 % erhöht – der feste Flächenton trägt jetzt den Hauptanteil der Farbe, der Weichzeichner bleibt für Kanten und etwas Tiefe, schlägt aber nicht mehr so stark auf den Gesamtton durch. Nicht am Gerät nachgeprüft (nur mit den eingesandten Screenshots vermessen).
