...
| 2026-09-18 | **Beobachtung 18: Bereichs-Pill-„Springen" behoben, Verdachts-Fix (v3.6.1) — war das falsche Element.** Seit mind. 6 Meldungen unverstandener Bug — Betreiber hat diesmal (ohne Video, aber mit brauchbaren Antworten) beschrieben: alle drei Tabs, Name+Position ändern sich, beim Bereichswechsel, Ausmaß abhängig von Namenslänge. Code-Befund schien die Vermutung zu bestätigen: `.bereich-pill` hatte nur `max-width`, keine feste Breite. Fix: feste Breite `min(62vw, 220px)`. War aber nicht die Ursache — Betreiber meinte die untere Navigationsleiste, nicht die Bereichs-Pill oben. |
| 2026-09-18 | **Beobachtung 18, echte Ursache gefunden und behoben (v3.6.3).** Betreiber lieferte zwei Screenshots mit eindeutigem Beweis: Bei kurzem Inhalt sitzt die untere Navigationsleiste sichtbar höher als bei langem/gescrolltem Inhalt — die ganze Leiste springt, nicht ein Button. Zwei eigene Theorien vorher durchgerechnet und mit einer isolierten Testseite widerlegt (Margin-Symmetrie, Icon-Varianten). Echte Ursache: mobile Adressleisten klappen beim Scrollen ein/aus, `.nav` (`position: fixed`) bezieht sich dabei nicht zuverlässig auf denselben sichtbaren Ausschnitt — `env(safe-area-inset-bottom)` kennt nur den Geräte-Notch, nicht die Browser-Toolbar. Fix: `syncViewportGap()` (`app.js`) synct `--vv-gap` über `window.visualViewport`, `.nav`s `bottom` bezieht diesen Wert ein. Ausdrücklich unbestätigt (kein Gerät mit echter Toolbar-Dynamik verfügbar), Betreiber-Test steht aus. Details in `beobachtungen-lernwerkzeug.md` Punkt 18 und `CHANGELOG.md` 3.6.3. |
| 2026-09-18 | **Beobachtung 18, dritter Anlauf: v3.6.3-Fix hat nicht geholfen, Theorie strukturell ausgeschlossen (v3.6.4).** Betreiber hat mit frisch neu installierter Home-Bildschirm-App (garantiert aktueller Code) denselben Sprung bestätigt — damit scheidet die visualViewport/Adressleisten-Theorie aus: Standalone-PWAs (`display: standalone`) haben keine Browser-Toolbar. Zusätzlich geprüft und ausgeschlossen: `body`s `min-height` (schon `100svh`), Containing-Block durch Transform/Filter/Contain auf einem `.nav`-Elternelement (keins gefunden). Statt vierter Vermutung: Debug-Overlay (`?debug=nav` in der URL) zeigt Live-Werte, damit die nächste Diagnose auf echten Zahlen statt Screenshots beruht. Nächster Schritt liegt beim Betreiber: URL mit Parameter öffnen, Screenshots der Werte in beiden Tab-Zuständen schicken. Details in `beobachtungen-lernwerkzeug.md` Punkt 18 und `CHANGELOG.md` 3.6.4. |
