...
| ~~14~~ | ~~Analytics – Ja oder Nein?~~ | **entschieden 24.09.2026: JA** (Betreiber: „datenschutzerklärung usw. lässt sich umschreiben [...] daher analytics"). Gebaut in v3.17.0, **aus bis zum Schlüssel**: Betreiber legt ein PostHog-Projekt (EU) an und trägt den Projektschlüssel in `app.js` (`POSTHOG_KEY`) ein bzw. schickt ihn. Datenschutzerklärung Punkt 15 ergänzt. **Rechtlich gegengelesen 24.09.2026** (Betreiber: „mein anwalt sagt dass das alles stimmt") — Punkt 15 dabei an zwei Stellen präzisiert (Transfer-Mechanismus, Hash-Formulierung), interne Interessenabwägung geschrieben: [`analytics/INTERESSENABWAEGUNG.md`](analytics/INTERESSENABWAEGUNG.md). **Verbleibend, rein organisatorisch:** Auftragsverarbeitungsvertrag mit PostHog abschließen, Transfer-Grundlage (SCC/DPF) dort prüfen, Datenaufbewahrung ≤ 1 Jahr im PostHog-Projekt einstellen — dann erst `POSTHOG_KEY` eintragen. Details: [`analytics/GERUEST.md`](analytics/GERUEST.md). |
