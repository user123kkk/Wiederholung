...
| 2026-09-18 | **Beobachtung 18 (Nav-Leiste springt auf dem Handy), drei Anläufe (v3.6.1–3.6.4), noch ungelöst.** Bereichs-Pill-Breite (falsches Element), `visualViewport`-Sync gegen Adressleisten-Dynamik (scheidet aus, Betreiber nutzt Standalone-Home-Bildschirm-App ohne Browser-Toolbar) — beide halfen nicht. Debug-Overlay (`?debug=nav`) eingebaut, nächster Schritt liegt beim Betreiber: Screenshots der Live-Messwerte schicken. Details in `beobachtungen-lernwerkzeug.md` Punkt 18. |
