...
**Status:** Blöcke 1–10 `fertig` (v3.1.0–3.4.3), **alle am echten Handy verifiziert.** Blöcke 8–10 kommen aus der Bildersammlung des Betreibers (17.09.2026), siehe [`BILDER-BEFUND.md`](BILDER-BEFUND.md). Offene Frage 6 ist seit 18.09.2026 dauerhaft geklärt (§7-Lockerung gilt grundsätzlich). Am 18.09.2026 zusätzlich die zwei liegen gebliebenen Nachlese-Punkte gebaut (v3.5.0): Werkzeugleiste Verwalten (Video 1) und Smart Default beim Speichern in eine Speicherkarte (Video 3) — **Betreiber-Test am Handy steht noch aus**, siehe `LOGBUCH.md`. Block 11 (v3.7.3, 22.09.2026) aus einem vierten Video: Gleit-Indikator beim Tab-Wechsel — **Betreiber-Test am Handy steht noch aus**. Block 12 (v3.7.4, 22.09.2026) aus einer eigenen UX-Sichtung ohne Video-Quelle (Betreiber-Auftrag „verbessere alles"): Hover-Rückmeldung in der Verwalten-Liste nachgezogen. Rückmeldung dazu: „sieht gleich aus" — zu Recht, eine Hover-Korrektur ist am Bildschirm kaum zu sehen. Block 13 (v3.7.5, 22.09.2026) liefert deshalb einen sichtbaren Tiefe-Durchgang über Schrift, Fortschritt, Einstellungen, Verwalten und den Google-Knopf. Betreiber-Test am echten Handy (Flugmodus) fand drei echte Fehler und zwei geklärte Nicht-Fehler — als **Block 14** behoben (v3.7.6): Zeitlimit gegen endloses Laden bei Netzausfall, gedämpfter Knopf-Glanz, sauberer gesperrter Google-Knopf. Details siehe `LOGBUCH.md`. **Betreiber-Test mit echtem Netz steht noch aus.** Am 22.09.2026 wurde zwischendurch eine **Gestaltung 4.0** (kompletter Neuaufbau im iOS-Stil) gebaut und auf Wunsch des Betreibers wieder zurückgenommen — der bestehende Stil (Creme auf Fast-Schwarz, Serifen-Überschriften, schwebende Leiste) bleibt die Basis, Verbesserungen laufen nur noch in kleinen Schritten darin. **Block 15** (v3.8.0) ist der erste solche Schritt: eine zusammenhängende Betreiber-Rückmeldung mit Screenshot, abgearbeitet in einem Durchgang. **Betreiber-Test am echten Handy steht noch aus**, besonders für das neue Wischen zwischen den Reitern.
...
| 12 | **Eigene UX-Sichtung — Hover-Lücke** | `fertig` (v3.7.4) | Kein Video, sondern Prüfung gegen UX-Gesetze (`PRINZIPIEN.md`, „Eigene UX-Sichtung 22.09.2026"): `.card-row` in Verwalten fehlte als einzige klickbare Zeile der App die Desktop-Hover-Rückmeldung. Nachgezogen. Weitere geprüfte Punkte: keine echte Lücke (siehe Tabelle dort) oder bewusst auf einen möglichen Block 13 verschoben (Verwalten-Liste bei großen Kartenmengen). |
| 13 | **Sichtbarer Tiefe-Durchgang** | `fertig` (v3.7.5) | Betreiber-Rückmeldung zu Block 12 („sieht gleich aus") → Auftrag, Schrift/Fortschritt/Einstellungen/Verwalten/Google-Knopf sichtbar zu verbessern, „mach was du willst". Neuer Token `--sheen` (Lichtschein auf jeder erhobenen Fläche), Lichtkante+Schatten auf dem einen gefüllten Knopf pro Bildschirm, Verlaufsfüllung auf großen Kennzahlen und dem Heute-Balken (Fortschritt), Icon-Chips in Einstellungen-Zeilen, markenkonformer heller Google-Knopf, Überschriften-Schnitt 600→700. Kein Eingriff in Lernlogik, keine neue Farbe, kein zweiter gefüllter Knopf pro Bildschirm. |
| 17 | **Wischen neu gefasst** | `fertig` (v3.8.2) | Betreiber: „das wischen ist sehr unangenehm und schwer, will wirklich was flüssiges". Ursache: Block 15 hatte die Bewegung auf 42 % der Fingerbewegung gedämpft. Jetzt 1:1-Verfolgung wie die beiden anderen (bewährten) Wisch-Gesten der App, plus Tempo-Kriterium (Fling), `requestAnimationFrame`-Bündelung, kontinuierlicher Ausflug statt Schnitt vor dem eigentlichen Wechsel, `ease-spring` beim Zurückfedern. Dabei zusätzlich vereinheitlicht: das Zurückfedern beim Blatt-Wegwischen trug noch die alte Federung. Details: `LOGBUCH.md`. |
| 16 | **Tastatur & doppelte Zahl** | `fertig` (v3.8.1) | Betreiber-Screenshot iPhone 11: die Bildschirmtastatur lag über dem Karten-Blatt, die Speichern-Knöpfe waren nicht erreichbar. Ursache: iOS verkleinert nur `visualViewport`, nicht das Layout, an dem `position:fixed` hängt — `dvh` hilft dabei nicht. Neu gemessen in `syncTastatur()`, als `--tastatur` an alle Blätter und Dialoge weitergegeben. Dazu: „Diese Woche N neue dazu." aus „Dein Stoff" entfernt (dieselbe Zählung stand zweimal weiter oben). Offene Frage an den Betreiber zur großen Zahl in „Dein Stoff", siehe `LOGBUCH.md`. |
| 15 | **Ruhe und Fluss** | `fertig` (v3.8.0) | Zusammenhängende Betreiber-Rückmeldung zu 3.7.6 („unübersichtlich", „billig", „hard reset", „Glanz", „soll zum Icon passen", „zwischen den Tabs wischen"). Verwalten entrümpelt (Überschrift raus, abgeschnittene Hinweiszeile behoben, Kasten um einen Knopf weg, Speicherkarten-Namen waren 0px breit) · Lichtkante nur noch auf dem einen gefüllten Knopf, `--sheen` halbiert · gleitender Reiter-Anzeiger als `::before` der überlebenden `.nav` statt Einblend-Keyframes · Wischen zwischen den drei Reitern mit Klick-Sperre danach · Seitenwechsel länger und weicher · Farbtonleiter warm wie `icon.svg` · „Dein Stoff"-Legende zweizeilig. Details: `LOGBUCH.md`. |
| 14 | **Anmeldung im Flugmodus geprüft** | `fertig` (v3.7.6) | Betreiber-Test am echten Handy (Flugmodus) mit zwei Screenshots. Drei echte Fehler behoben: `mitZeitlimit()` gegen endloses Laden bei Netzausfall (nicht bei Google/Apple-Popup, siehe Begründung im Code), Knopf-Glanz aus Block 13 gedämpft, eigener `:disabled`-Zustand für den Google-Knopf statt verwaschenem Grau. Zwei Punkte geklärt, kein Fehler: E-Mail-Fehlermeldungen waren schon vorhanden (`AUTH_ERRORS`), „lernkarte" bei der Google-URL ist die Firebase-Projekt-ID (nicht änderbar ohne Migration). Zwei Punkte offen: violetter Balken am Bildschirmrand vermutlich Browser-Erweiterung, nicht diese App; Spam-Zustellung der Bestätigungsmails braucht einen Firebase-Konsolen-Schritt. |
