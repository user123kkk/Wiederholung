...
(async () => {
  ...
  await cdp.send("Profiler.enable"); await cdp.send("Profiler.start"); for (const q of ["كتاب","Schlussel"]) {
  ...
  { const pr = (await cdp.send("Profiler.stop")).profile; const m = new Map(pr.nodes.map(n => [n.id, n])); const s = {}; pr.samples.forEach((id, i) => { const n = m.get(id); const k = n.callFrame.functionName || "(anon)"; s[k] = (s[k] || 0) + (pr.timeDeltas[i] || 0) / 1000; }); console.log("Selbstzeit:", Object.entries(s).sort((a, c) => c[1] - a[1]).slice(0, 10).map(([k, v]) => k + " " + v.toFixed(0)).join(", ")); }
