...
(async () => {
  ...
  await p.click('#app .btn-known');
  for (const t of [60, 180, 300]) { await setze(p, t); await foto(p, 'g-' + t); }
  await p.evaluate(() => document.getAnimations().forEach(a => { try { a.finish(); } catch (e) { a.cancel(); } })); await p.waitForTimeout(700);
  await p.click('#app .study-flaeche'); await p.evaluate(() => document.getAnimations().forEach(a => { try { a.finish(); } catch (e) { a.cancel(); } })); await p.waitForTimeout(100);
  await p.click('#app .btn-unknown'); await setze(p, 180); await foto(p, 'g-nicht');
  await p.evaluate(() => document.getAnimations().forEach(a => { try { a.finish(); } catch (e) { a.cancel(); } })); await p.waitForTimeout(700);
  await p.click('#app .study-flaeche'); await p.evaluate(() => document.getAnimations().forEach(a => { try { a.finish(); } catch (e) { a.cancel(); } })); await p.waitForTimeout(100);
  await p.click('#app .btn-almost'); await setze(p, 180); await foto(p, 'g-fast');
